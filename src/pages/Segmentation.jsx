    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
        {segments.map((seg, index) => (
          <div key={index} className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition">
            <div className={`w-12 h-12 ${seg.bg} ${seg.color} rounded-lg flex items-center justify-center text-2xl mb-4`}>
              {seg.icon}
            </div>
            ...
            <div className="flex justify-between items-center border-t border-gray-100 pt-4">
              ...
              <span className="text-lg font-bold text-slate-800">{seg.count}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      >
        <form className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Nama Segmen</label>
            <input type="text" className="w-full px-4 py-2 bg-gray-50 border rounded-lg outline-none" placeholder="Contoh: Pasien Hipertensi" />
          </div>
          <div>
